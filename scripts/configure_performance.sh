...
configure_nvme_boot() {
    ...
    if command -v raspi-config &> /dev/null; then
        ...
        log_info "Setting boot order to NVME first via raspi-config..."
        raspi-config nonint do_boot_order B2
        log_success "Boot order set to NVME first"
    else
        log_warn "raspi-config not available, manual EEPROM configuration may be needed"
        log_info "You may need to manually configure boot order in raspi-config later"
    fi
}
